...
public class App_fetchRandomItems {
    ...
    public static void main( String[] args )
    {
        ...
        try (CosmosClient client = new CosmosClientBuilder()
            ...
            //
            // Fetch a random set of IDs from the container
            List<String> ids = fetchRandomItems(container, 10);
    ...
    //
    // Create list with random amount of entries from the existing container
    private static List<String> fetchRandomItems(CosmosContainer container, int numberOfRandomItems) {
               
        // Assuming items have an 'id' field you can select. Adjust the query as needed.
        String query = "SELECT c.id FROM c";
        CosmosQueryRequestOptions queryOptions = new CosmosQueryRequestOptions();
        List<String> allIds = new ArrayList<>();
    
        // Fetch items as JsonNode
        CosmosPagedIterable<JsonNode> iterable = container.queryItems(query, queryOptions, JsonNode.class);
        iterable.forEach(item -> {
            // Ensure there's an 'id' field and it's not null
            if (item.has("id") && !item.get("id").isNull()) {
                allIds.add(item.get("id").asText());
            }
        });
    
        // Randomly select items from the list of IDs
        Random random = new Random();
        List<String> randomIds = new ArrayList<>();
        for (int i = 0; i < numberOfRandomItems; i++) {
            if (allIds.isEmpty()) break; // Ensure we don't run into an error if there are fewer items than requested
            randomIds.add(allIds.remove(random.nextInt(allIds.size())));
        }
